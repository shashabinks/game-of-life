...
func gameOfLife(p golParams, keyChan <-chan rune) []cell {
	...
	sendChans := make([]chan uint8,  p.threads)
	for s := range sendChans{
		sendChans[s] = make(chan uint8, p.imageWidth) // buffer
	}
	receiveChans := make([]chan uint8, p.threads)
	for r := range receiveChans{
		receiveChans[r] = make(chan uint8, p.imageWidth)
	}
	topChans := make([]chan uint8, p.threads)
	for t := range topChans{
		topChans[t] = make(chan uint8, p.imageWidth)
	}
	bottomChans :=make([]chan uint8, p.threads)
	for b := range bottomChans{
		bottomChans[b] = make(chan uint8, p.imageWidth)
	}

	turnChs := make([]chan int, p.threads)
	for t := range turnChs{
		turnChs[t] = make(chan int)
	}

	go distributor(p, dChans, aliveCells, sendChans, receiveChans, turnChs,keyChan)
	go pgmIo(p, ioChans)

	for i := 0; i < p.threads; i++{
		go worker(p,sendChans[i],receiveChans[i],topChans[i],bottomChans[i],topChans,bottomChans, turnChs[i], i)
	}
	alive := <-aliveCells
	return alive
}
